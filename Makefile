...
# Change Version Number and RELEASE NOTES
	...
	cd ${REPO_DIR} && sed -i -e 's/ONDEWO_SURVEY_VERSION.*=.*/ONDEWO_SURVEY_VERSION = ${ONDEWO_SURVEY_API_VERSION}/' Makefile
	cd ${REPO_DIR} && sed -i -e 's/ONDEWO_PROTO_COMPILER_GIT_BRANCH.*=.*/ONDEWO_PROTO_COMPILER_GIT_BRANCH=tags\/${PROTO_COMPILER}/' Makefile
	cd ${REPO_DIR} && sed -i -e 's/SURVEY_API_GIT_BRANCH.*=.*/SURVEY_API_GIT_BRANCH=tags\/${ONDEWO_SURVEY_API_VERSION}/' Makefile && head -30 Makefile
