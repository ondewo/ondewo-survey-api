...
CURRENT_RELEASE_NOTES=`cat RELEASE.md \
	| sed -n '/Release ONDEWO Survey Server API ${ONDEWO_SURVEY_API_VERSION}/,/\*\*/p'`
...
githubio_logic:
	...
	@! cat ondewo.github.io/data.js | sed -n "/name\: '${REPO_NAME_UPPER}'/,/end\: ''/p" | grep -q "number: '${ONDEWO_SURVEY_API_VERSION}'" || (echo "Already Released" && exit 1)
	...
	$(eval TEMP_TEXT:= $(shell cat ondewo.github.io/script_object.txt | sed -e "s/VERSION/${ONDEWO_SURVEY_API_VERSION}/g" -e "s/TECHNOLOGY/${REPO_NAME}/g"))
	...
	@mkdir "${DOCS_DIR}"
	@cp docs/* ${DOCS_DIR}
	@sed -i "s/h1>Protocol Documentation/h1>${REPO_NAME_UPPER} ${ONDEWO_SURVEY_API_VERSION} Documentation/" ${DOCS_DIR}/index.html

	$(eval HEADER_LINE:= $(shell cat ${DOCS_DIR}/index.html | grep -n "${REPO_NAME_UPPER} ${ONDEWO_SURVEY_API_VERSION} Documentation" | grep -o -E '[0-9]+' | head -1 | sed -e 's/^0\+//'))
...
release_all_clients:
	@make release_python_client || (echo "Already released ${ONDEWO_SURVEY_API_VERSION} of Python Client")
	@make release_nodejs_client || (echo "Already released ${ONDEWO_SURVEY_API_VERSION} of Nodejs Client")
	@make release_typescript_client || (echo "Already released ${ONDEWO_SURVEY_API_VERSION} of Typescript Client")
	@make release_angular_client || (echo "Already released ${ONDEWO_SURVEY_API_VERSION} of Angular Client")
	@make release_js_client || (echo "Already released ${ONDEWO_SURVEY_API_VERSION} of JS Client")
...
# Change Version Number and RELEASE NOTES
	...
	cd ${REPO_DIR} && sed -i -e 's/ONDEWO_SURVEY_VERSION.*=.*[0-9]*.[0-9]*.[0-9]/ONDEWO_SURVEY_VERSION = ${ONDEWO_SURVEY_API_VERSION}/' Makefile
	...
	cd ${REPO_DIR} && sed -i -e 's/SURVEY_API_GIT_BRANCH=tags\/[0-9]*.[0-9]*.[0-9]/SURVEY_API_GIT_BRANCH=tags\/${ONDEWO_SURVEY_API_VERSION}/' Makefile && head -30 Makefile
